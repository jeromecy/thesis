...
\section{Proof of Tractor Spline Properties}
\begin{proof}
...
It is easily seen that the matrices $B$ and $C$ having the following property that 
\begin{align*}
& BB^\top=CC^\top=I_n, \\
& C^\top CB^\top=B^\top BC^\top=0.
\end{align*}
Denoting $G=B^\top B+ \gamma C^\top C +n\Omega_\lambda$ and given $\hat{\theta} =(B^\top B+ \gamma C^\top C +n\Omega_\lambda)^{-1}(B^\top y+\gamma C^\top v)$, we have $G\hat{\theta}=B^\top y+\gamma C^\top v$ and 
\begin{align*}
& BG\hat{\theta}=y+\gamma BC^\top v\\
& CG\hat{\theta}=CB^\top y+\gamma v.
\end{align*}
Further, $C^\top CG=nC^\top C\Omega_\lambda$, which is the matrix consisting of the even rows of $\Omega_\lambda$. 
