...
\cite{septier2009multiple} discuss some attractive features of genetic algorithms and simulated annealing into the framework of MCMC based particle scheme. One may refer to the reference for details. 
...
%\item{Step 3.} Resample $\left\lbrace  \theta_{t+1}^{(i)},x_{t+1}^{(i)},s_{t+1}^{(i)}  \right\rbrace$ according to $w_{t+1}$
...
Resample $\left\lbrace  \theta_{k}^{(i)},x_{k}^{(i)},s_{k}^{(i)}  \right\rbrace$ according to $\tilde{w}_{k}$.\\
...
\For{$k=1,\ldots,t$}{
\For{$i=1,\ldots,N$}{
Resample $\tilde{z}_k^{\left(i\right)}=\left(\tilde{s}_k^{x\left(i\right)},\tilde{s}_k^{\left(i\right)},\tilde{\theta}^{\left(i\right)}\right)$ from $z_k^{(i)} = \left( s_k^x,s_k,\theta\right)^{(i)}$ with weight $w_k\propto p\left(y_{k+1}\mid z_t^{(i)} \right)$.\\
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{Chapters/04Filtering/plot/plotlyFilterCompX.png}
...
The estimates for $x_t$ from $t=300$ to $897$ are very close and the differences are hard to tell visually. See Figure \ref{FilterRiewComparesion02}. 
