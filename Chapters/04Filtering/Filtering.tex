...
The effective sample size $N_{\mbox{\scriptsize eff}}$ is suggested to monitor how bad the degeneration is, which is defined as 
\begin{align*}
...
%\item{Step 3.} Approximate likelihood function by \eqref{esembleKalmanLikeli}. 
%\item{Step 4.} Update. Draw $\theta$ either use normal approximation or grid-based approximation to find \eqref{jointposteriorterm2}. 
%\item{Step 5.} Draw $\theta^{(i)}\sim p(\theta\mid y_{1:t})$.
%\item{Step 6.} Generate forecast ensemble by $x_t^{f(i)} = x_t^{p(i)}+w_t$.
%\item{Step 7.} Draw posterior ensemble using \eqref{ensembleKalmanForecast}
%\end{itemize}
\begin{algorithm}[h]
\SetAlgoLined 
%\KwResutt{Write here the resutt }
...
Update. Draw $\theta$ either use normal approximation or grid-based approximation to find \eqref{jointposteriorterm2}. \\
Draw $\theta^{(i)}\sim p(\theta\mid y_{1:k})$.\\
Generate forecast ensemble by $x_k^{f(i)} = x_k^{p(i)}+w_k$.\\
Draw posterior ensemble using \eqref{ensembleKalmanForecast}.
}
}
