...
&= N\left(\frac{1}{2}(y_{t+1}+\phi x_t),\frac{1}{\sqrt{2}}\right)
\end{align*}
\item Step 3. Update sufficient statistics  $s_{t+1}=S(\tilde{s}_{t}, x_{t+1},y_{t+1})$.
\begin{align*}
s_{t+1,1} &= x_{t+1} \\
...
\includegraphics[width=0.9\textwidth,height=9cm]{Chapters/04Filtering/plot/plotlyFilterCompX.png}
\caption{A simulation study shows that the three algorithms converge to the true $\phi$ at similar speed. The filtering for $x$ is very competitive.} \label{FilterRiewComparesion}
\end{figure}

\begin{table}[]
\centering
\caption{Comparing MSE and computation time of the three algorithms by running 100 iterations with $N=5\,000$ particles. }
...
From figure (\ref{FilterRiewComparesion}) it can be seen that the three algorithms converge to the true parameter $\phi$ with similar speed along time $t$. It is hard to tell which one is better visually. The filtering estimations for $x$ are competitive. However, from table (\ref{FilterRiewComparesionTable}), we can see that the Particle Learning returns the smallest mean squared error (MSE) at 0.5823, which is better than the other two algorithms. Liu and West's filter runs faster than others and has a smaller variance on parameter estimation. 
...
Subject to pages and time, some algorithms are not included. For instance, readers can refer to unscented Kalman filter \cite{wan2000unscented} for non-linear estimation, particle MCMC \cite{andrieu2010particle} for off-line Bayesian estimation, on-line gradient approach \cite{poyiadjis2005maximum} for parameter estimation. 
