...
The filtering problem is to maximize the posterior $p(x_t\mid y_{1:t})$. By assuming the current state $x_t$ only depends on the previous one step $x_{t-1}$, the joint distribution is 
\begin{equation*}
...
Suppose the particles $x_1, \cdots, x_N$ are drawn from the target pdf $q(x)$
...
\subsection*{Unscented Particle Filter}
...
%For $i = 1, \cdots, N$, update the prior $t-1$ distribution for each particle with the UKF:
%\begin{align*}
%&\mbox{Calculate sigma points:}\\
%& X_{t-1}^{(i)\alpha}=[ \bar{x}_{t-1}^{(i)\alpha}, \bar{x}_{t-1}^{(i)\alpha}+\gamma\sqrt{P_{t-1}^{(i)\alpha}}, \bar{x}_{t-1}^{(i)\alpha}-\gamma\sqrt{P_{t-1}^{(i)\alpha}} ] \\
%&\mbox{Propagate particle into future (time update)}:\\
%& X_{t\mid t-1}^{(i)x}=F(X_{t-1}^{(i)x},u_t,_{t-1}^{(i)v}).
%\end{align*}
