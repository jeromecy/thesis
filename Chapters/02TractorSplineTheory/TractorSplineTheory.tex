...
Now, focus on an observation $y_i$ at point $t_i$ as being a new observation by omitting it from the set of data, which are used to estimate $\hat{f}$. Denote by $\hat{f}^{(-i)}(t,\lambda)$ the estimated function from the remaining data, where $\lambda$ is the smoothing parameter. Then $\hat{f}^{(-i)}(t,\lambda)$ minimizing 
...
\frac{1}{n}\sum_{j \neq i}\left(y_j-f(t_j) \right)^2+\lambda\int (f'')^2dt,
\end{equation}
and $\lambda=\frac{n-1}{n}\lambda^*$ can be quantified by cross-validation score function
\begin{equation}
...
For a Tractor spline and its MSE function, there are two parameters $\lambda$ and $\gamma$ to be estimated for. Therefore, the objective function (\ref{originalcv}) becomes
\begin{align*}
\frac{1}{n-1}\sum_{j \neq i}\left( y_j-f(t_j) \right)^2+\frac{\gamma^*}{n-1}\sum_{j \neq i} \left( v_j-f'(t_j) \right)^2+ \int \lambda^*(t) \left( f'' \right)^2dt,
\end{align*}
which is equivalent to 
\begin{align}
\frac{1}{n}\sum_{j \neq i}\left( y_j-f(t_j) \right)^2+\frac{\gamma}{n}\sum_{j \neq i} \left( v_j-f'(t_j) \right)^2+ \int \lambda(t) \left( f'' \right)^2dt,
\end{align}
$\gamma=\frac{n-1}{n}\gamma^*$, $\lambda(t)=\frac{n-1}{n}\lambda^*(t)$ and the cross-validation score function is
\begin{align}
...
Then
\begin{align}
\mathbf{\hat{f}}^{(-i)}&=S\mathbf{y}_i^*+\gamma T\mathbf{v}_i^*\\
\mathbf{\hat{f}}'^{(-i)}&=U\mathbf{y}_i^*+\gamma V\mathbf{v}_i^*
\end{align}
\end{lemma}
