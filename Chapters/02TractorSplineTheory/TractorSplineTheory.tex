...
Table \ref{tablecompareSNR} is comparing the capability of Tractor spline in retrieving the true SNR. 

\begin{table}
	\centering
	\caption{SNR.}\label{tablecompareSNR}
	\setlength\tabcolsep{1.5pt}
	\begin{tabular}{|c|c| c| c|}
\hline	                   & true SNR & from $f$ & from fitted $\hat{f}$ \\ \hline
		\textit{Blocks}    & 7   & 6.9442    &  6.9485     \\ \hline
		\textit{Blocks}    & 3   &  2.9761   &               \\ \hline
		\textit{Bumps}     & 7  & 6.9442    &  6.9548  \\ \hline
		\textit{Bumps}     & 3  & 2.9761    &              \\ \hline
		\textit{HeaviSine} & 7 & 6.9442    &   6.9207   \\ \hline
		\textit{HeaviSine} & 3 & 2.9761    &             \\ \hline
		\textit{Doppler}   & 7   & 6.9442   &  6.8757   \\ \hline
		\textit{Doppler}   & 3   & 2.9761   &               \\ \hline
	\end{tabular}	
\end{table}
...
Table \ref{1dxymse} illustrates the MSE of all methods on both $x$ and $y$ axes. The proposed Tractor spline returns the smallest errors among all methods.
...
%\begin{sidewaystable}
\begin{table}
\caption{Mean square error. Tractor spline returns smallest errors among all these methods. P-spline was unable to reconstruct the $y$ trajectory as the original dataset contains 0 $\Delta_y$.} \label{1dxymse}
% \centering
	\setlength\tabcolsep{1.5pt}
\begin{center}
	...
	\textit{$x$}   &  0.2046 & 0.2830 & 0.3298     & 2860.5480   & 256.0494  & 6.2959  \\ \hline
	\textit{$y$}   &  0.0020 & 0.3062 & 0.3115     & \textit{NA} & 1960.2220 & 19.3330  \\ \hline
 	\end{tabular}
 \end{center}
\end{table}
%\end{sidewaystable} 
...
\begin{figure}
    ...
    \caption{Distribution of penalty values on $x$ and $y$}\label{penaltyxyggXYLine}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
    \centering
    ...
    \caption{Histograms of penalty values on $x$ and $y$}\label{penaltyxyggXYHist}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
    \centering
    ...
    \caption{Reconstruction with penalty values on $x$ and $y$ separately}\label{penaltyxyggXYPath}
    \end{subfigure}
 \caption{Penalty function of Tractor spline on $x$ and $y$ axes. The big black dots in figure \ref{penaltyxyggXYPath} indicate large penalty values. It can be seen that most of large penalty values occur at turnings, where the tractor likely slows down and takes breaks. }\label{penaltyxygg}
 \end{figure}


\subsection{2-Dimension Trajectory}

In a 2-dimensional trajectory reconstruction, we use the same parameters $\lambda_d$, $\lambda_u$ and $\gamma$ on both $x$ and $y$ axes. The parameters will be found by cross-validation where it returns the smallest score of
\begin{equation}
...
%  \caption{Fitted data points on $x$ and $y$ axes. The mean square errors (MSE) on $x$ and $y$ are 0.240734 and 0.478422 respectively. The mean distance error $\sqrt{(\hat{f}_x-x)^2+ (\hat{f}_y-y)^2}$ is 0.645830.}\label{2dxy}
%\end{figure}
