...
In this chapter, we propose an adaptive smoothing spline method based on Hermite spline basis functions to obtain a reconstruction of $f$ and $f'$ from noisy data $\mathbf{y}$ and $\mathbf{v}$. Rather than only using residuals of the form $f(t_i)-y_i$ in the objective function in equation (\ref{objective}), we include residuals of $f'(t_i)-v_i$ in a new term containing a new parameter $\gamma$. In this way, the spline keeps a balance on both observed $\mathbf{y}$ and $\mathbf{v}$. Using the new basis functions, we reconstruct a smoothing spline on the whole interval $[a,b]$. Derived from the new objective function, an advanced cross validation formula of $f(t)$ and $f'(t)$ is given. We show that it performs very well on simulated signal data \textit{Blocks}, \textit{Bumps}, \textit{HeaviSine} and \textit{Doppler}. This method can be used in either getting true signal from noisy data or reconstructing the trajectory of a moving object. 
...
In a 2D curve nonparametric regression, consider $n$  time points $t_{1:n}$, such that $a \leq t_1, \dots, t_n \leq b$. Let $z_i=(x_i,y_i)$ and $w_i=(u_i,v_i)$ for $i=1, \cdots, n$. We define a positive piecewise constant function $\lambda(t)$ :
\begin{equation}
...
\begin{figure}
  ...
  \begin{subfigure}{0.45\textwidth}
    \centering
    \includegraphics[width=\linewidth,height=0.45\textwidth]{Chapters/02TractorSplineTheory/plot/ggplot/ggBlocksPSpline.pdf}
    \caption{Reconstruction by tractor spline with normal penalty term.}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
    \centering
...
\begin{table}
	\centering
	\caption{TMSE. True mean square errors of different methods. The numbers in bold indicate the smallest error among these methods under the same level. The proposed tractor spline returns the smallest TMSE among all the methods under the same level except for $\mathit{Doppler}$ with SNR=7. The differences are significant. }\label{tmse3200}
	\setlength\tabcolsep{1.5pt}
	\begin{tabular}{|c|c| c| c| c|c|c|c|}
...
%\begin{sidewaystable}
\begin{table}
\caption{Mean square error. Tractor spline returns smallest errors among all these methods. P-spline was unable to reconstruct the $y$ trajectory as the original dataset contains some time differences equal to 0.} \label{1dxymse}
% \centering
	\setlength\tabcolsep{1.5pt}
\begin{center}
...
The penalty function of proposed tractor spline is
...
\end{equation}
In 2-dimensional reconstruction, $\Delta d$ in adjusted penalty term is the Euclidean distance $d(p_1,p_2)=\sqrt{(\Delta x)^2+(\Delta y)^2}$ between two position on 2D surface.

Similar results as that in 1 dimension reconstruction, the velocity information keeps trajectory in the right direction and the penalty term makes sure that the crazy curve will disappear between long time gap points. Figure \ref{2dxy} demonstrates fitted trajectory on $x$ and $y$ axises separately and compares the final 2D trajectory with the line based original one. 
\begin{figure}
  \centering
