...
In a general process, we know that $p(\mathbf{y},\mathbf{v}\mid f) = N(f,\Gamma)$. However, we are more interested in $f$ given measurements 
\begin{equation}
...
Observing $y_i\sim N\left(f (t_i),\sigma^2\right)$ and $v_i\sim N\left(f (t_i),\frac{\sigma^2}{\gamma}\right)$, $i=1,\ldots,n$, the joint distribution of $\mathbf{y},\mathbf{v}$ and $f(t)$ is normal with mean zero and a covariance matrix can be found by the following 
\small
\begin{align*}
...
\end{align*}
\normalsize where $R_0(s,t)$ and $R_1(s,t)$ are taken from \eqref{TractorSplineKernelR0} and \eqref{TractorSplineKernelR1}. %, and $\beta$ is constant.

Therefore, by using a standard result on multivariate normal distribution (such as Result 4.6 in \citep{johnson1992applied}), the posterior mean of $f(t)$ is seen to be 
...
\end{equation}
%where $\mathbf{d},\mathbf{c}$ and $\mathbf{b}$ are coefficient vectors. 
Thus, by substituting to the equation \eqref{maineq2}, it can be written as 
\begin{align}\label{GassianProcessRawequation2}
%\begin{split}
%\begin{split}
%&\frac{1}{n}\sum_{i=1}^{n} \left( y_i - d_1-d_2t_i-\sum_{j=1}^{n}c_jR_1(t_j,t_i)-\sum_{j=1}^{n}b_j\dot{R}_1(t_j,t_i)-\rho(t_i) \right) ^2\\
%&\frac{\gamma}{n}\sum_{i=1}^{n} \left( v_i - d_2-\sum_{j=1}^{n}c_jR'_1(t_j,t_i)-\sum_{j=1}^{n}b_j\dot{R}'_1(t_j,t_i)-\rho'(t_i) \right) ^2\\
%+&\sum_{i=1}^{n-1}\lambda_i \int_{t_i}^{t_{i+1}}  \left( \sum_{j=1}^{n}c_jR''_1(t_j,t)+\sum_{j=1}^{n}b_j\dot{R}''_1(t_j,t)+\rho''(t)\right)^2dt
%\end{split} \\
\begin{split}
...
%\begin{align}
%\sum_{i=1}^{n-1}w_i\int_{t_i}^{t_{i+1}}c_kR_1''(t_k,t)c_lR_1''(t_l,t)dt =\sum_{i=1}^{n-1} w_i\int_{t_i}^{t_{i+1}}c_k w_i^{-1}(t_k-t)_+c_lw_i^{-1}(t_l-t)_+dt =c_kR_1(t_k,t_l)c_l\\
%\sum_{i=1}^{n-1}w_i\int_{t_i}^{t_{i+1}}c_kR_1''(t_k,t)b_l\dot{R}_1''(t_l,t)dt =\sum_{i=1}^{n-1} w_i\int_{t_i}^{t_{i+1}}c_k w_i^{-1}(t_k-t)_+b_lw_i^{-1}\mathbf{1}(t_l)dt=c_k\dot{R}_1(t_k,t_l)b_l\\
%\sum_{i=1}^{n-1}w_i\int_{t_i}^{t_{i+1}}b_k\dot{R}_1''(t_k,t)b_l\dot{R}_1''(t_l,t)dt =\sum_{i=1}^{n-1} w_i\int_{t_i}^{t_{i+1}}b_k w_i^{-1}\mathbf{1}(t_k)b_lw_i^{-1}\mathbf{1}(t_l)dt=b_k\dot{R}'_1(t_k,t_l)b_l
%\end{align}
%\begin{equation}
%\sum_{i=0}^{n}w_i\int_{t_i}^{t_{i+1}}c_kR_1''(t_k,t)c_lR_1''(t_l,t)dt =\sum_{i=0}^{n} w_i\int_{t_i}^{t_{i+1}}c_k w_i^{-1}(t_k-t)_+c_lw_i^{-1}(t_l-t)_+dt =c_kR_1(t_k,t_l)c_l
%\end{equation}
%\begin{equation}
%\sum_{i=0}^{n}w_i\int_{t_i}^{t_{i+1}}c_kR_1''(t_k,t)b_l\dot{R}_1''(t_l,t)dt =\sum_{i=0}^{n} w_i\int_{t_i}^{t_{i+1}}c_k w_i^{-1}(t_k-t)_+b_lw_i^{-1}\mathbf{1}(t_l)dt=c_k\dot{R}_1(t_k,t_l)b_l
%\end{equation}
%\begin{equation}
%\sum_{i=0}^{n}w_i\int_{t_i}^{t_{i+1}}b_k\dot{R}_1''(t_k,t)b_l\dot{R}_1''(t_l,t)dt =\sum_{i=0}^{n} w_i\int_{t_i}^{t_{i+1}}b_k w_i^{-1}\mathbf{1}(t_k)b_lw_i^{-1}\mathbf{1}(t_l)dt=b_k\dot{R}'_1(t_k,t_l)b_l
%\end{equation}
\begin{align}
...
\end{equation}
Because $f=\sum_{i=1}^{2n}\theta_iN_i\left(t\right)$ is a linear combination of basis functions, extended to the solution with covariance matrices, the parameter is found by 
\begin{equation}
