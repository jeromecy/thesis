...
In fact, suppose the covariance functions for $y$, $v$ and $f$, $f'$ are \small
\begin{align*}
...
\Cov \left(\dot{f}_1,f_1\right)  &= \dot{R}_1\left(s,t\right) =\beta \left(t+\int_0^s\left(t-u\right)_+du \right)
\end{align}
...
\frac{1}{n}\sum_{i=1}^{n}\left(y_i-f(t_i)\right)^2+\frac{\gamma}{n}\sum_{i=1}^{n}\left(v_i-f'(t_i)\right)^2+\sum_{i=0}^{n}\lambda_i\int_{t_i}^{t_{i+1}}f''(t)^2dt.
\end{equation}
...
&\frac{1}{n}\sum_{i=1}^{n} \left( y_i - d_1-d_2t_i-\sum_{j=1}^{n}c_jR_1(t_j,t_i)-\sum_{j=1}^{n}b_j\dot{R}_1(t_j,t_i)-\rho(t_i) \right) ^2\\
+&\frac{\gamma}{n}\sum_{i=1}^{n} \left( v_i - d_2-\sum_{j=1}^{n}c_jR'_1(t_j,t_i)-\sum_{j=1}^{n}b_j\dot{R}'_1(t_j,t_i)-\rho'(t_i) \right) ^2\\
+& \sum_{i=0}^{n}\lambda_i \int_{t_i}^{t_{i+1}}  \left( \sum_{j=1}^{n}c_jR''_1(t_j,t)+\sum_{j=1}^{n}b_j\dot{R}''_1(t_j,t)+\rho''(t)\right)^2dt. 
\end{split}
%\end{split}
\end{align}
Because of orthogonality, $\rho(t_i) = \left(R_1(t_i,\cdot),\rho\right)=0$, $\rho'(t_i) = \left(\dot{R}_1(t_i,\cdot),\rho'\right)=0$, $i=1,\ldots,n$. 
...
Similarly, by denoting the matrix $\{S\}_{ij}=(t_i)^{j-1}$, $j=1,2$, $\{Q\}_{ij}=R_1(t_j,t_i)$, $\{P\}_{ij}=\dot{R}_1(t_j,t_i)$ and $\{P'\}_{ij}=\dot{R}'_1(t_j,t_i)$, the above equation \eqref{GassianProcessRawequation2} becomes the matrix form 
