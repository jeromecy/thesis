...
Consider a regression problem with observations $y_i = f(t_i)+\varepsilon_i$, $i=1,\ldots,n$, containing i.i.d normal noises $\varepsilon_i\sim N(0,\sigma^2)$ in the space $C^{(m)}[0,1]=\{ f:f^{(m)}\in \mathit{L}_2[0,1] \}$. The classic nonparametric or semi-parametric regression is a function that minimizes the following penalized least square functional 
...
Indeed, for every $f=f_0+f_1$ on $[0,1]$ with $f_0$ and $f_1$ having independent Gaussian priors with zero means and covariances as 
\begin{align*}
\E (f_0(s)f_0(t)) &= \tau^2 R_0(s,t)=\tau^2 \sum_{\nu=0}^{m-1}\frac{s^\nu}{\nu!}\frac{t^\nu}{\nu!}, \\
\E (f_1(s)f_1(t)) &= bR_1(s,t) = b\int_{0}^{1} \frac{(s-u)_+^{m-1}}{(m-1)!} \frac{(t-u)_+^{m-1}}{(m-1)!},
\end{align*}
where $R_0$ and $R_1$ are from (\ref{GaussianProcessKernelR}). Because of the observations are normal distributed as $y_i\sim N(f(t_i),\sigma^2)$, then the joint distribution for $y_{1:n}$ and $f(t)$ is normal with zero mean and the covariance matrix is 
...
\section{Tractor Spline as Bayes Estimation}


Recall the definition of Tractor spline introduced in Chapter \ref{ChapterTractorSpline}. It is the solution to the objective function (\ref{tractorsplineObjective}), where an extra term for $f'(t)-v$ and an extra parameter $\gamma$ are incorporated. 

In this section, we are going to prove that the Tractor spline is corresponding to Bayesian estimation in a particular reproducing kernel Hilbert space. 


\subsection{Reproducing Kernel Hilbert Space $\mathcal{C}_{p.w.}^{2}[0,1]$}

The space $C^{(m)}[0,1]=\{ f:f^{(m)}\in \mathit{L}_2[0,1] \}$ is a set of functions $f$ whose $m$th derivatives are square integrable on the domain $[0,1]$. For a tractor spline, it only requires $m=2$. In fact, its second derivative is piecewise linear. Besides, only if $\gamma$ is constant, the second derivative is continuous at joint knots. Then we introduce the space 
\begin{equation*}
\mathcal{C}_{p.w.}^{2}[0,1]=\{f: f''\in \mathit{L}_2[0,1], f,f' \mbox{ are continuous and } f'' \mbox{ is piecewise linear}\}.
\end{equation*}
...
\end{equation}
in the space $\mathcal{C}_{p.w.}^{2}[0,1]$ is a tractor spline. Equipped with an appropriate inner product
\begin{equation}
\langle f,g \rangle=f(0) g(0)+f'(0) g'(0)+\int_{0}^{1}f''g''dx,
\end{equation}
the space $\mathcal{C}_{p.w.}^{2}[0,1]$ is made a reproducing kernel Hilbert space. In fact, the representer $R_s(\cdot)$ is 
\begin{equation}\label{kerneleq}
R_s(t)=1+st+\int_{0}^{1} (s-u)_+(t-u)_+du.
\end{equation}
It can be seen that $R_s(0)=1, R'_s(0)=s$, and $R''_s(t)=(s-t)_+$.

The two terms of the reproducing kernel $R(s,t)=R_s(t)=R_0(s,t)+R_1(s,t)$, where
\begin{align}
R_0(s,t)&=1+st \\
...
\end{equation*}
with inner product $\langle f,g \rangle_1=\int_{0}^{1}f''g''dt$. Thus, $\mathcal{H}_0$ and $\mathcal{H}_1$ are two subspaces of the $\mathcal{C}_{p.w.}^{2}[0,1]$, and the reproducing kernel is $R_s(\cdot) = R_0(s,\cdot)+R_1(s,\cdot)$.


Define a new notation $\dot{R}(s,t)=\frac{\partial R}{\partial s}(s,t)=\frac{\partial R_0}{\partial s}(s,t)+\frac{\partial R_1}{\partial s}(s,t)=t+\int_0^s(t-u)_+du$. Obviously $\dot{R}_s(t) \in \mathcal{C}_{p.w.}^{2}[0,1]$. Additionally, we have $\dot{R}_s(0)=0, \dot{R}'_s(0)=\frac{\partial \dot{R}_s}{\partial t}(0)=1$, and $ \dot{R}''_s(t)=\begin{cases}
0 & s\leq t \\ 1 & s>t \end{cases}$. Then, for any $f\in \mathcal{C}_{p.w.}^{2}[0,1]$, we have 
\begin{equation*}
...
\mathcal{A}=\{f: f=\sum_{j=1}^{n}\alpha_jR_1(t_j,\cdot)+\sum_{j=1}^{n}\beta_j\dot{R}_1(t_j,\cdot)\} 
\end{equation*}
is a closed linear subspace of $\mathcal{H}_1$. Then we have a new space $\mathcal{H}_*=\mathcal{\dot{H}} \cup \mathcal{A}$. Thus the two new sub spaces in $\mathcal{C}_{p.w.}^2[0,1]$ are $\mathcal{H}_0$ and $\mathcal{H}_*$.

For any $f  \in \mathcal{C}_{p.w.}^2[0,1]$, it can be written as 
\begin{equation}\label{GaussianProcessFunctionF}
f(t)=d_1+d_2t+\sum_{j=1}^{n}c_jR_1(t_j,t)+\sum_{i=j}^{n}b_j\dot{R}_1(t_j,\cdot) +\rho(t)
\end{equation}
where $\mathbf{d},\mathbf{c}$ and $\mathbf{b}$ are coefficients, and $\rho(t) \in \mathcal{H}_1 \ominus \mathcal{H}_*$. 
...
+&\lambda \int_0^1 \left( \sum_{j=1}^{n}c_jR''_1(t_j,t)+\sum_{j=1}^{n}c_j\dot{R}''_1(t_j,t)+\rho''(t)\right)^2dt
\end{split}
\end{equation}
By orthogonality, $\rho(t_i) = (R_1(t_i,\cdot),\rho)=0$, $\rho'(t_i) = (\dot{R}_1(t_i,\cdot),\rho')=0$, $i=1,\ldots,n$. By denoting that 
\begin{align*}
...
Now in the model $y=f(t)+\varepsilon$ and $v=f '(t)+\frac{\varepsilon}{\gamma}$ where $\varepsilon \sim N(0,\sigma^2)$, according to equation $(\ref{GaussianProcessFunctionF})$, for $f(t) \in \mathcal{C}_{p.w.}^{2}[0,1]$, we have
\begin{equation}
...
Observing $y_i\sim N(f (t_i),\sigma^2)$ and $v_i\sim N(f (t_i),\frac{\sigma^2}{\gamma})$, the joint distribution of $\mathbf{y},\mathbf{v}$ and $f(t)$ is normal with mean zero and a covariance matrix can be found. The posterior mean of $f(t)$ is 
...
First of all, consider observations $y=f(t)+\varepsilon_1$ and $v=f'(t)+\varepsilon_2$, where $\varepsilon_1\sim N(0,\sigma^2W^{-1})$, $\varepsilon_2\sim N(0,\frac{\sigma^2}{\gamma}U^{-1})$ with unknown variance parameter $\sigma^2$. A tractor spline $\hat{f}$ with correlated errors in space $\mathcal{C}_{p.w}^2[0,1]$ is the minimizer of 
\begin{equation}
...
\end{equation*}
Followed by the approximation $S_{ii}\approx\frac{1}{n}\mbox{tr}(S)$, $T_{ii}\approx\frac{1}{n}\mbox{tr}(T)$, $U_{ii}\approx\frac{1}{n}\mbox{tr}(U)$ and $V_{ii}\approx\frac{1}{n}\mbox{tr}(V)$ \cite{syed2011review}, the tractor spline GCV will be 
\begin{equation*}
...
With the same parameter found by GCV, Tractor spline and its Bayesian estimates return the same reconstruction. However, because we are using constant $\lambda$ rather than a piecewise $\lambda(t)$, the Tractor spline might not be the optimal solution compared with the one containing an adaptive penalty term in section \ref{ChapterTractorSpline}. 
...
\includegraphics[width=0.8\textwidth,height=8cm]{Chapters/03GPR/plot/ggsim_cov} 
 \caption{Comparing Tractor spline and its Bayesian estimates. Two methods are corresponding to each other. In this figure, black dots indicate observations and dark line stands for the reconstruction. The Tractor spline and its posterior $\E(f(t) \mid \mathbf{y}, \mathbf{v})$ of Gaussian process regression give the same results. }
\end{figure}
...
In this Chapter, we take a review on the relationship between smoothing spline and the Bayesian estimates of the work have been done in the last few years. With improper priors, the two methods are corresponding to each other. By following the work that has been done by \cite{gu2013smoothing}, we find the Bayesian estimate for Tractor spline with a constant penalty parameter $\lambda$.  However, it is a particular scenario of Tractor spline with $\lambda(t)$ varying on domains and might not be an optimal solution. Additionally, we give the formula of GCV for Tractor spline with correlated errors on $y$ and $v$. 
