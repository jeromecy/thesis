...
A Gaussian process is a collection of random variables, any finite number of which have a joint Gaussian distribution \cite{b_gpml}. Gaussian process regression is a probability distributions over functions. It is fully defined by its mean $m(t)$ and covariance $K(s,t)$ functions as
\begin{align*}
m(t)&=\E [f(t)] \\
...
Recall the definition of Tractor spline introduced in Chapter \ref{ChapterTractorSpline}. It is the solution to the objective function (\ref{tractorsplineObjective}), where an extra term for $f'(t)-v$ and extra parameter $\gamma$ are incorporated. 

In this section, we are going to prove that the Tractor spline is corresponding to Bayesian estimation in a reproducing Hilbert space. 

\subsection{Reproducing Kernel Hilbert Space on $\mathcal{C}_{p.w.}^{2}[0,1]$}

The space $C^{(m)}[0,1]=\{ f:f^{(m)}\in \mathit{L}_2[0,1] \}$ is a set of functions $f$ whose $m$th derivatives are continuous on the domain $[0,1]$. For a tractor spline, it only requires $m=2$. In fact, its second derivative is piecewise continuous. Then we introduce the space
\begin{equation*}
\mathcal{C}_{p.w.}^{2}[0,1]=\{f:f,f' \mbox{ are continuous and } f'' \mbox{ is piecewise continuous on } [0,1] \}.
\end{equation*}
...
\end{equation}
It can be seen that $R_s(0)=1, R'_s(0)=x$, and $R''_s(t)=(s-t)_+$.
...
According to Theorem \ref{theoremKernel}, $R_0$ can correspond the space of polynomials $\mathcal{H}_0=\{f:f''=0\}$ with an inner product $(f,g)_0= f(0)g(0)+f'(0)g'(0)$, and $R_1$ corresponds the orthogonal complement of $\mathcal{H}_0$
\begin{equation*}
\mathcal{H}_1=\{f:f(0)=0, f'(0)=0, \int_{0}^{1}f''^2dt<\infty\}
\end{equation*}
with inner product $(f,g)_1=\int_{0}^{1}f''g''dt$. Thus, $\mathcal{H}_0$ and $\mathcal{H}_1$ are two subspaces of the $\mathcal{C}_{p.w.}^{2}[0,1]$, and the reproducing kernel is $R_s(\cdot) = R_0(s,\cdot)+R_1(s,\cdot)$.
...
0 & s\leq t \\ 1 & s>t \end{cases}$. Then, for ant $f\in \mathcal{C}_{p.w.}^{2}[0,1]$, it gives us 
\begin{align*}
(\dot{R}_s,f) &=\dot{R}_s(0)f(0)+\dot{R}'_s(0)f'(0)+\int_0^1\dot{R}''_s f''	 du=f'(0)+\int_0^t f''du=f'(t).
\end{align*}
It can be seen that the first term $\dot{R}_0=t\in \mathcal{H}_0$, and the space spanned bt the second term  $\dot{R}_1=\int_0^s(t-u)_+du$, denoted as $\mathcal{\dot{H}}$, is not in $\mathcal{H}_1$, but $\mathcal{\dot{H}} \cap \mathcal{H}_1\neq \emptyset$. Then we have a new space $\mathcal{H}_*=\mathcal{\dot{H}} \cup \mathcal{H}_1$. Thus the two new sub spaces in $\mathcal{C}_{p.w.}^2[0,1]$ are $\mathcal{H}_0$ and $\mathcal{H}_*$.
...
\end{split}
\end{align*}
By orthogonality, $\rho(t_i) = (R_1(t_i,\cdot),\rho)=0$, $\rho'(t_i) = (\dot{R}_1(t_i,\cdot),\rho')=0$, $i=1,\ldots,n$. Denoting by
\begin{align*}
...
\end{align*}
and noting that $\int_0^1R''_1(t_i,t)R''_1(t_j,t)dt=R_1(t_i,t_j)$, $\int_0^1R''_1(t_i,t)\dot{R}''_1(t_j,t)dt=\int_0^{v}(t_i-t)dt=\dot{R}_1(t_j,t_i)$, and $\int_0^1\dot{R}''_1(t_i,t)\dot{R}''_1(t_j,t)dt=\int_0^{v}1dt=\dot{R}'_1(t_i,t_j)$, where $v=\mbox{min}(t_i,t_j)$, the above equation can be written as
...
\end{split}
\end{equation}
Note that $\rho$ only appears in the third term in $(\ref{matriteq})$, which is minimized at $\rho=0$. Hence, a polynomial smoothing spline resides in the space $\mathcal{H}_0\oplus \mathcal{A}$ of finite dimension. Then the solution to $(\ref{maineq})$ could be computed via minimization of the first two terms in $(\ref{matriteq})$ with respect to $\mathbf{d}$, $\mathbf{c}$ and $\mathbf{b}$.
...
Now in the model $y=f(t)+\varepsilon$ and $v=f '(t)+\frac{\varepsilon}{\gamma}$ where $\varepsilon \sim N(0,\sigma^2)$, according to equation $(\ref{etaeq})$, for $f(t) \in \mathcal{C}_{p.w.}^{2}[0,1]$ and $t \in T$, we have
\begin{equation}
...
Observing $y_i\sim N(f (t_i),\sigma^2)$ and $\mathbf{v}_i\sim N(f (t_i),\frac{\sigma^2}{\gamma})$, the joint distribution of $\mathbf{y},\mathbf{v}$ and $f(t)$ is normal with mean zero and a covariance matrix can be found. The posterior mean of $f(t)$ is 
...
\end{split}
\end{align}
where $\phi$ is $2 \times 1$ matrix with entry $1$ and $t$, $\xi$ is $n\times 1$ matrix with $i$th entry $R(t_i,t)$ and $\psi$ is $n\times 1$ matrix with $i$th entry  $\dot{R}(t_i,t)$, $\rho=\tau^2/\beta$ and $n\lambda =\sigma^2/\beta$. 
...
Now in the model $y=f(t)+\varepsilon_1$ and $v=f'(t)+\varepsilon_2$ where $\varepsilon_1\sim N(0,\sigma^2W^{-1})$, $\varepsilon_2\sim N(0,\frac{\sigma^2}{\gamma}U^{-1})$, and $\sigma^2$ is unknown. A tractor spline $\hat{f}$ in space $\mathcal{C}_{p.w}^2[0,1]$ is the minimizer of
\begin{equation}
...
\begin{figure}[h]
 ...
 \caption{Comparing two methods. In this graph, the blue line is reconstruction from tractor spline, the red line is the mean of Gaussian Process, which is the posterior $\mathbb{E}(f(t) \mid \mathbf{y}, \mathbf{v})$.}
\end{figure}
...
Given $\mathbf{t}$ a length-10 sequence from 0 to 1, 
\begin{align*}
...
\begin{figure}[h]  
	...
	\begin{tabular}{c}
		...
		\small (b) 
	\end{tabular}
	\caption{(a) Comparing two methods under the same parameters $\lambda=0.01$ and $\gamma=0.1$. In this graph, the blue line is reconstruction from tractor spline, the red line is the mean of Gaussian Process, which is the posterior $\E (f(t) \mid  \mathbf{y}, \mathbf{v})$. (b) The differences between two methods under the same parameters.}
\end{figure}
